...
- **Web-unavailable fallback:** if the web search tool fails after 2 attempts, write the report from model knowledge with the SAME format, mark unverifiable facts TENTATIVE, note "WEB RESEARCH UNAVAILABLE — generated from model knowledge" at the top of the report AND in your report. Downstream executors must not be blocked by the tool.
...
- **Trimming search output** — never pipe web_search.sh through tail/head/less/more/grep -m; trimmed results lose sources
